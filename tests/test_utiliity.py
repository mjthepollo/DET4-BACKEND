...
from setup import BUCKET_NAME, openai, s3
from utility import (add_assistant_message_to_messages,
                     add_user_message_to_messages, create_output_file_name,
...
def check_file_in_s3_bucket(bucket_name, file_key):
    try:
        ...
        if e.response['Error']['Code'] == '404':
            ...
            print(f"Error checking file '{file_key}' in bucket '{bucket_name}': {e}")
        return False

def test_save_file_to_s3():
    test_file_path = "tests/src/test.txt"
...
def test_decode_audio():
    ...
    test_output_name = f"tests/src/test_output_{now_second}.mp3"
    with open(test_output_name, "wb") as f:
        f.write(audio_data)
    assert os.path.exists(test_output_name), f"The file '{test_output_name}' does not exist"
...
def test_add_user_message_to_messages():
    ...
    result_messages = add_user_message_to_messages(existing_messages,new_message)
    assert len(result_messages)==4
    assert result_messages[3]["role"] == "user"
    assert result_messages[3]["content"] == new_message


def test_add_assistant_message_to_messages():
    existing_messages = [
        {"role":"system", "content":"This is just test"},
        {"role":"user", "content":"Hello World?!"},
    ]
    new_message = "GoodBye World!"
    result_messages = add_assistant_message_to_messages(existing_messages,new_message)
    assert len(result_messages)==3
    assert result_messages[2]["role"] == "assistant"
    assert result_messages[2]["content"] == new_message


def test_get_message_by_chatgpt():
    test_quote = "Hello world!"
    start_time = time.time()
    send_messages = [{"role":"user", "content":f"THIS IS API_TEST! You must tell me '{test_quote}'"}]
    received_message = get_message_by_chatgpt(send_messages)
    end_time = time.time()
    elapsed_time = end_time - start_time
    print("Generate Text By CHATGPT elapsed: {:.2f} seconds".format(elapsed_time))
...
def test_get_audio_file_url_using_polly():
    ...
    end_time = time.time()
    elapsed_time = end_time - start_time
    print("Generate AUDIO By POllY elapsed: {:.2f} seconds".format(elapsed_time))
    assert check_file_in_s3_bucket(BUCKET_NAME, test_file_name)
    print(f"GENERATED AUDIO File by POLLY URL: {url}\n(!!!!Deleted Very After Text!!!!)")
    s3.delete_object(Bucket=BUCKET_NAME, Key=test_file_name)
