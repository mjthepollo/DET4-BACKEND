...
from papago_test import translate
from setup import openai
...
# create variables to collect the stream of chunks
    ...
    full_reply_content = ''.join([m.get('content', '') for m in collected_messages]).strip()
    ...
    print(f"(Stream)Full response received {spent_time:.2f} seconds after request")
...
def use_non_stream_answer(message):
    ...
    print(f"(Non-Stream)Full response received {spent_time:.2f} seconds after request")
...
with open('output_compare.csv', mode='w', newline='') as csv_file:
    ...
    header_row = ["Input", "Stream-output", "Stream-response-time","Non-stream-output", "Non-stream-response-time", "Translated Input", "Stream-output","Stream-response-time", "Non-stream-output", "Non-stream-response-time"]
